-- PM+ edit a completed work_session.
-- Accepts corrected clock-in/out times, break minutes, notes, and mileage.
-- Recomputes gross/net hours, CA overtime tiers, and labor cost.
-- Syncs the linked time_entries row (if one exists).
...
BEGIN
  ...
  -- ── Recompute hours (only if session is complete) ─────────────────────────
  IF p_clocked_out_at IS NOT NULL THEN
    ...
    -- Preserve the original is_seventh_day flag (editing times doesn't change the work week)
    ...
    -- Wage on session date (use original user, not caller)
